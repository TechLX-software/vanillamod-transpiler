...
vMod.executeCallback = function (
  ...
  let splitContext =
    typeof context === "string" ? (splitContext = context.split(" ")) : context;
  let splitConditions =
    typeof conditions === "string"
      ? (splitConditions = conditions.split(" "))
      : conditions;
  let splitOutputs =
    typeof outputs === "string" ? (splitOutputs = outputs.split(" ")) : outputs;

  return ["execute"].concat(
    splitContext,
    splitConditions,
    splitOutputs,
...
vMod.executeShort = function (executeAsEntity, commandToBeExecuted) {
  return ["execute", "as", executeAsEntity, "at", "@s", "run"].concat(
    commandToBeExecuted
  );
};
