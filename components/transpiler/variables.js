...
vars.Variable = class extends vars.Declared {
  ...
  constructor(declarator, variableType, mcFunctionContents) {
    ...
    this.blockScope = `${pathTemp.replace(/[:|/]/g, ".")}-depth-${
      declarator.depth
    }`;
  ...
  getVariable(limit = false) {
    ...
    const optionalLimiter =
      this.variableType == "drone" || limit ? ",limit=1" : "";
...
vars.Drone = class extends vars.Variable {
  ...
  action(commandToBeRun) {
    const selector = `@e[tag=${this.reference}]`;
  ...
  createInGameVariable() {
    ...
    return [
      `summon ${this.entityType} ~ ~ ~ ${dataTags}`,
      "function vanillamod-library:drone-summon",
    ];
  }
...
vars.Function = class {
  ...
  createClickableFunctionChatJSON(mod, functionName) {
    ...
    const tellrawJSON =
      `["",{"text":"${
        mod.initFunctionsList.length + 1
      }: ","bold":true},{"text":"${functionName}","color":"blue","underlined":true,` +
