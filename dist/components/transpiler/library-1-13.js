...
vMod.moveForwards = function (distance) {
  return ["teleport", "@s", "^ ^ ^".concat(distance)];
};
...
vMod.command = function (fullCommand) {
  console.log("ARGS OF COMMAND:", fullCommand); // validate if string or esprima literal object
...
vMod.executeCallback = function (context, conditions, outputs, callbackArgs, runCommandDefinition) {
  ...
  var splitContext = typeof context === "string" ? splitContext = context.split(" ") : context;
  var splitConditions = typeof conditions === "string" ? splitConditions = conditions.split(" ") : conditions;
  var splitOutputs = typeof outputs === "string" ? splitOutputs = outputs.split(" ") : outputs;
...
vMod.team = function (action, teamName, param1, optionValue) {
  if (teamName.charAt(0) == "@") {
    // checks if team entry is variable.
    teamName = teamName.slice(teamName.indexOf("=") + 1, -1); // Will need to this more extensively when teams are changed to tags
  }
...
//# sourceMappingURL=library-1-13.js.map